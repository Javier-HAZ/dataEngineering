...
def main(params):
    ...
    os.system(f'curl {url.strip()} -o ny_taxi_postgres_data/{file_name}')
    print('\n')
    ...
    if '.csv' in file_name:
        df = pd.read_csv(f"ny_taxi_postgres_data/{file_name}", nrows=10)
        df_iter = pd.read_csv(file_name, iterator=True, chunksize=100000)
    elif '.parquet' in file_name:
        file = pq.ParquetFile(f"ny_taxi_postgres_data/{file_name}")
